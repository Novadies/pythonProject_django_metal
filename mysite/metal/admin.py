...
class Whattafuu_Filter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset): # собственно фильтр. Можно добавлять больше двух значений
        if self.value() == 'long':
...
class Metal_infoAdmin(admin.ModelAdmin):
    ...
    list_per_page = 218
    actions = ['something']
    search_fields = ['steel', 'metals_class__steel_class'] # metals_class внешний ключ, необходимо указать поле явно
    ...
    @admin.action(description="Что-то там сделать") #добавление кастомного метода для строк в бд
